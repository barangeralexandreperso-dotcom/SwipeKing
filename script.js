...
if(distance<firstSwitch){swipeMode="vertical";arena.classList.remove("horizontal");return;}
if(level%2===0){
swipeMode="vertical";
arena.classList.remove("horizontal");}
else{swipeMode="horizontal";arena.classList.add("horizontal");}
}
function swipe(e){if(!holding || gameOver)return;let deltaV=e.clientY-lastY;let deltaH=e.clientX-lastX;
if(swipeMode==="vertical"){if(Math.abs(deltaV)>6){distance += deltaV * gainMultiplier;beep(260);}
}
else{if(Math.abs(deltaH)>6){distance += deltaH * gainMultiplier;beep(300);}}
lastY=e.clientY;
lastX=e.clientX;
}
function update(){d.textContent=Math.floor(distance)+" cm";updateMode();distance -= enemyForce;if(distance < 0){distance = 0;}
...
async function endGame(){if(gameOver)return;gameOver=true;let score=Math.floor(distance);
...
window.addEventListener("touchstart",e=>{holding=true;lastY=e.touches[0].clientY;lastX=e.touches[0].clientX;},{passive:false});
window.addEventListener("touchmove",e=>{swipe(e.touches[0]);},{passive:false});
